...
@Controller
public class ManagerBoardInsertController {
	@Autowired
	private ManagerBoardInsertService boardInsertService;
	
	@RequestMapping(value="/managerFolder/Manager_Board_Insert.do", method= {RequestMethod.GET, RequestMethod.POST})
								...
								HttpServletRequest request,MultipartHttpServletRequest mtfRequest) throws Exception
	{
		...
		System.out.println("request.getParameter(\"board_title\")="+request.getParameter("board_title"));
		...
		mnv.setViewName("redirect:Manager_Board_SelectList.do?section=1&page=1&board_class="+board_class);
