...
<body>
		...
		<div class="wrapper_p">
			<div class="buyListBody">
				...
				<c:forEach var="sl" items="${selectList}" varStatus="status">
					<div class="buyListBox">
						<a href="BuyGoodsSelectOne.do?buy_index=${sl.buy_index }" style="margin-top:0px; display:block;"><img src=".${sl.goods_imageTitle }"  alt="상품이미지" class="buyImage"/></a>
						...
						<c:if test="${sl.del_state eq '입금대기중'}">
							<a class="goods_price" style="border:1px solid black; cursor:pointer;" onclick="pay('${sl.buy_index}','${page}','${memberVO.member_id}','${sl.goods_lastprice}')">입금하기</a>
			...
			<div class="pagenum">
				...
				<ul style="display:inline-block; list-style:none;">
					<c:if test="${page > 9}">
						<li style="float:left;"><a href="BuyGoodsList.do?page=${startPage - 9}&member_id=${memberVO.member_id}" style="float:left; position:relative; ">이전으로</a></li>
					</c:if>
					<c:set var="i" value = "5"/>
					<c:forEach begin="${startPage }"  end="${endPage }" var = "i" varStatus="status" step="1">
						<c:if test="${status.count <= 9 }">
							<li style="float:left;"><a href="BuyGoodsList.do?page=${i}&member_id=${memberVO.member_id}" style="float:left; position:relative; ">[${i}]</a></li>
						</c:if>
					...
					<c:if test="${ endeqpage > eqpage }">
							<li style="float:left;"><a href="BuyGoodsList.do?page=${startPage+9}&member_id=${memberVO.member_id}" style="float:left; position:relative; ">다음으로</a>	</li><!-- 페이지는 페이지가 움직임 -->
					</c:if>
				</ul>
				</div>
	...
	<script>
		function pay(buy_index,page,member_id,goods_lastprice)
		{
			if(confirm("정말 구매하시겠습니까?"))
			{
				location.href = "BuyGoodsPay.do?buy_index="+buy_index+"&page="+page+"&member_id="+member_id+"&goods_lastprice="+goods_lastprice;
			}
			else
			{
				return;	
