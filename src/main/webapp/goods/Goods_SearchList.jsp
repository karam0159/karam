	...
	<div class="wrapper">	<!-- sidemenuul만빼서 c:if로 할까? -->
		...
		<div class="wrapper_p">
			<div class="GoodsFullBox">
				<c:forEach var="gv" items="${goodsVO}" varStatus="status">
					<div class="GoodsBox">
						<a href="GoodsSelectOne.do?goods_index=${gv.goods_index}&page=1"><img src=".${gv.goods_imageTitle}" alt="이미지사진" class="goodsbox_img"/></a><br>
						<a href="GoodsSelectOne.do?goods_index=${gv.goods_index}&page=1" class="goodsTitle">${gv.goods_name}</a><br>
						<a class="goodsPrice">${gv.goods_lastprice }</a>
					</div>
				</c:forEach>	
			</div>
			<div class="pagenum">
				...
				<ul style="display:inline-block; list-style:none;">
					<c:if test="${page > 9}">
						<li style="float:left;"><a href="GoodsSearch.do?page=${startPage - 9}&goods_name=${goods_name}" style="float:left; position:relative; ">이전으로</a></li>
					</c:if>
					<c:set var="i" value = "5"/>
					<c:forEach begin="${startPage }"  end="${endPage }" var = "i" varStatus="status" step="1">
						<c:if test="${status.count <= 9 }">
							<li style="float:left;"><a href="GoodsSearch.do?page=${i}&goods_name=${goods_name}" style="float:left; position:relative; ">[${i}]</a></li>
						</c:if>
					...
					<c:if test="${ endeqpage > eqpage }">
							<li style="float:left;"><a href="GoodsSearch.do?page=${startPage+9}&goods_name=${goods_name}" style="float:left; position:relative; ">다음으로</a>	</li><!-- 페이지는 페이지가 움직임 -->
					</c:if>
				</ul>
				</div>
